  ...
  describe "ESpec.Assertions.ContainExactly" do
    context "Success" do
      ...
      it "checks success with `not_to`" do
        ...
        expect(message) |> to(eq "`[:a, 1, :five, \"x\"]` doesn't contain exactly `[\"x\", :a, :five, 5]`.")
      end

      it "checks success with `to`" do
        message = expect([{:a, 1}, 5, {:b, "16"}]).to contain_exactly([{:b, "16"}, {:a, 1}, 5])
        expect(message) |> to(eq "`[{:a, 1}, 5, {:b, \"16\"}]` contains exactly `[{:b, \"16\"}, {:a, 1}, 5]`.")
      end
      ...
      context "with `to`" do
        before do
          {:shared,
            ...
            message: "Expected `[{:a, 1}, {:a, 1}, 2]` to contain exactly `[{:a, 1}, 2]`, but it doesn't."}
        end
      ...
      context "with `not_to`" do
        before do
          {:shared,
            ...
            message: "Expected `[1, 2, 2, 3, 3]` not to contain exactly `[3, 1, 2, 2, 3]`, but it does."}
        end
