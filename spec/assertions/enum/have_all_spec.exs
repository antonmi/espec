  ...
  context "Error" do
    it "checks error with `to`" do
      try do
        ...
        error in [ESpec.AssertionError] ->
          ...
          |> to(end_with "to return `true` for all elements in `1..3`, but it returns `false` for some.")
      end
    end
    ...
    it "checks error with `not_to`" do
      try do
        ...
        error in [ESpec.AssertionError] ->
          expect(error.message)
          |> to(end_with "not to return `true` for all elements in `1..3`, but it returns `true` for all.")
      end
    end
  end
end
