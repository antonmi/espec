defmodule ESpec.Assertions.DatesTimes.BeSpec do
  ...
  describe "ESpec.Assertions.Be" do
    context "without specified granularity" do
      ...
      context "Errors with NaiveDateTime" do
        context "with `to`" do
          before do
            {:shared,
              expectation: fn -> expect(~N[2017-08-07 01:10:10]).to be :<, ~N[2017-08-07 01:10:15] end,
              message: "Expected `~N[2017-08-07 01:10:10] < ~N[2017-08-07 01:10:15]` to be `true` but got `false`."}
          end
        ...
        context "with `not_to`" do
          before do
            {:shared,
              expectation: fn -> expect(~N[2017-08-07 01:10:10]).to_not be :>, ~N[2017-08-07 01:10:15] end,
              message: "Expected `~N[2017-08-07 01:10:10] > ~N[2017-08-07 01:10:15]` to be `true` but got `false`."}
          end
        ...
        context "with `to`" do
          before do
            {:shared,
              ...
              message: "Expected `#{inspect datetime1()} < #{inspect datetime2()}` to be `true` but got `false`."}
          end
        ...
        context "with `not_to`" do
          before do
            {:shared,
              ...
              message: "Expected `#{inspect datetime1()} < #{inspect datetime2()}` to be `false` but got `true`."}
          end

          it_behaves_like(CheckErrorSharedSpec)
        end
      end
    end # without granularity

    context "specified with granularity" do
