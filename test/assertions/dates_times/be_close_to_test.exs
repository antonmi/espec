defmodule DatesTimes.BeCloseToTest do
  ...
  defmodule SomeSpec do
    ...
    context "Success, NaiveDateTime with granularity" do
      ...
      it do: expect(~N[2017-08-07 01:10:10]).to_not be_close_to(~N[2020-08-07 01:10:10], {:years, 2})
      it do: expect(~N[2017-08-07 01:10:10]).to be_close_to(~N[2017-08-14 01:10:10], {:weeks, 1})
      it do: expect(~N[2017-08-07 01:10:10]).to_not be_close_to(~N[2020-08-07 01:10:10], {:weeks, 2})
    end
    ...
    context "Success, Time with granularity" do
      ...
      it do: expect(~T[01:10:10.000001]).to_not be_close_to(~T[01:10:11.000002], {:microseconds, 1})
    end

    context "Success, DateTime with granularity" do
      let :datetime_a, do: DateTime.Extension.from_naive!(~N[2017-08-07 01:10:10.000001], "Etc/UTC")
      let :datetime_b, do: DateTime.Extension.from_naive!(~N[2020-08-07 01:10:10.000001], "Etc/UTC")
    ...
    context "Errors, NaiveDateTime with granularity" do
      it do: expect(~N[2017-08-07 01:10:10]).to be_close_to(~N[2017-08-07 01:10:15], {:seconds, 3})
      it do: expect(~N[2017-08-07 01:10:10]).to_not be_close_to(~N[2017-08-07 01:10:15], {:seconds, 5})
    end

    context "Errors, Time with granularity" do
      it do: expect(~T[01:10:10.000001]).to be_close_to(~T[01:10:10.000006], {:microseconds, 3})
      it do: expect(~T[01:10:10.000001]).to_not be_close_to(~T[01:10:10.000006], {:microseconds, 5})
    end
  ...
  setup_all do
    ...
    {:ok,
      success: Enum.slice(examples, 0, 35),
      errors: Enum.slice(examples, 36, 43)}
  end
