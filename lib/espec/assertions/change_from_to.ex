defmodule ESpec.Assertions.ChangeFromTo do
  ...
  defp success_message(subject, [func, before, value], _result, positive) do
    ...
    "`#{inspect(subject)}` #{to} the value of `#{inspect(func)}` from `#{inspect(before)}` to `#{
      inspect(value)
    }`."
  end
  ...
  defp error_message(subject, [func, before, value], {then, _initial, true, false}, positive) do
    ...
    "Expected `#{inspect(subject)}` #{to} change the value of `#{inspect(func)}` from `#{
      inspect(before)
    }` to `#{inspect(value)}`, but the value is `#{then}`."
  end
  ...
  defp error_message(subject, [func, before, value], {_then, initial, false, _}, positive) do
    ...
    "Expected `#{inspect(subject)}` #{to} change the value of `#{inspect(func)}` from `#{
      inspect(before)
    }` to `#{inspect(value)}`, but the initial value is `#{initial}`."
  end
end
